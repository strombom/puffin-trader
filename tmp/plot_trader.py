...
for episode_idx in range(start, stop + 1):
    ...
    with open('C:\\development\\github\\puffin-trader\\tmp\\log\\bitmex_sim_' + str(episode_idx) + '.csv') as csvfile:
    ...
    plot_wallet, = host_value.plot(rows[:,1], color='tab:blue', label="")
    ...
    host_value.set_ylabel("Realised value")
    ...
    fig.savefig('trading\\trade_' + str(episode_idx) + '.png', dpi=140)
    plt.close()
    #plt.show()
