...
{
    const auto features_height = features.size(2);
    const auto local_features = features.argmax(2).cpu();

    //const auto s = local_features.narrow(0, 15000, 25); // .argmax(1);
    //const auto m = local_features.max();

    //std::cout << s.argmax(2) << std::endl;

    //return;

    assert(prices.size() == img_width);

    const auto price_height = 512;
    ...
    //for (auto y = 0; y < features_height; y++) {
        for (auto x = 0; x < img_width; x++) {
            const auto y = local_features[features_start_idx + x][0].item().toInt();
            const auto c = 255;
