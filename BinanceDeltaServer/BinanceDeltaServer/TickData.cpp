...
TickData::TickData(void)
{
    for (auto &&symbol: Binance::symbols) {
        auto tick_array = std::make_unique<std::array<Tick, Binance::buffer_size>>();
...
void TickData::append(const std::string& symbol, time_point_ms timestamp, float price, float volume, bool buy, long long trade_id)
{
    ...
    const auto idx = buffer_next_idx.at(symbol) % Binance::buffer_size;
...
std::unique_ptr<std::vector<Tick>> TickData::get(const std::string& symbol, time_point_ms timestamp, int max_rows)
{
    ...
    //timestamp = ticks.at(symbol).get()->at((Binance::buffer_size + buffer_next_idx.at(symbol) - buffer_count.at(symbol) / 2) % Binance::buffer_size).timestamp();
    const auto timestamp_ms = timestamp.time_since_epoch().count();

    const auto last_idx = (Binance::buffer_size + buffer_next_idx.at(symbol) - 1) % Binance::buffer_size;
    const auto first_idx = buffer_count.at(symbol) < Binance::buffer_size ? 0 : buffer_next_idx.at(symbol);
    ...
    while (true) {
        ...
        if (read_ticks->at(idx).timestamp_ms < timestamp_ms) {
            if (idx != last_idx) {
                start_idx = (idx + 1) % Binance::buffer_size;
            }
            break;
        }
        if (idx == first_idx) {
        ...
        idx = (Binance::buffer_size + idx - 1) % Binance::buffer_size;
    }
