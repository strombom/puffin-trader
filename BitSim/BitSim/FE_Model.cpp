...
torch::Tensor FeatureEncoderImpl::forward(torch::Tensor observations)
{
    ...
    const auto chunks = observations.transpose(1, 2).reshape({ batch_size * n, BitSim::FeatureEncoder::n_channels, BitSim::FeatureEncoder::observation_length }).chunk(chunk_count);
    ...
    for (auto chunk_idx = 0; chunk_idx < chunks.size(); ++chunk_idx) {
        ...
        //std::cout << chunks[chunk_idx].sizes() << std::endl;
        ...
        //std::cout << feature.sizes() << std::endl;
        features.narrow(0, chunk_idx * first_chunk_size / n, chunk_size / n) = feature;
    }
