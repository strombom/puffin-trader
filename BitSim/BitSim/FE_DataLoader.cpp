    ...
    for (auto batch_idx = 0; batch_idx < batch_size; ++batch_idx) {
        ...
        for (auto obs_idx = 0; obs_idx < BitSim::n_observations; ++obs_idx) {
            ...
            for (auto feature_idx = 0; feature_idx < BitSim::observation_length; ++feature_idx) {
                ...
                batch.past_observations[batch_idx][BitSim::ch_price][obs_idx][feature_idx] = price_log;
                batch.past_observations[batch_idx][BitSim::ch_buy_volume][obs_idx][feature_idx] = row->vol_buy;
                batch.past_observations[batch_idx][BitSim::ch_sell_volume][obs_idx][feature_idx] = row->vol_sell;
            }
        }
        ...
        for (auto pred_idx = 0; pred_idx < BitSim::n_predictions * BitSim::n_positive; ++pred_idx) {
            ...
            for (auto feature_idx = 0; feature_idx < BitSim::observation_length; ++feature_idx) {
                ...
                batch.future_positives[batch_idx][BitSim::ch_price][pred_idx][feature_idx] = price_log;
                batch.future_positives[batch_idx][BitSim::ch_buy_volume][pred_idx][feature_idx] = row->vol_buy;
                batch.future_positives[batch_idx][BitSim::ch_sell_volume][pred_idx][feature_idx] = row->vol_sell;
            }
        }
        ...
        for (auto pred_idx = 0; pred_idx < BitSim::n_predictions * BitSim::n_negative; ++pred_idx) {
            ...
            for (auto feature_idx = 0; feature_idx < BitSim::observation_length; ++feature_idx) {
                const auto row = &intervals.rows[(int)((long)negative_index + feature_idx)];
                const auto price_log = std::log2f(row->last_price / first_price);

                batch.future_negatives[batch_idx][BitSim::ch_price][pred_idx][feature_idx] = price_log;
                batch.future_negatives[batch_idx][BitSim::ch_buy_volume][pred_idx][feature_idx] = row->vol_buy;
                batch.future_negatives[batch_idx][BitSim::ch_sell_volume][pred_idx][feature_idx] = row->vol_sell;
            }
        }
    }
