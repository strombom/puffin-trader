    ...
    def step(self, ie_idx: int, mark_price: float, mark_price_prev: float, slope: pd.DataFrame) -> bool:
        ...
        if self.direction == PositionDirection.short:
            ...
            if mark_price > threshold or \
                    (mark_price > slope['y1'] and slope['angle'] > angle_threshold and slope['angle'] > self.prev_slope_angle and mark_price > mark_price_prev) or \
                    (mark_price > slope['y1'] and slope['angle'] > 0 and slope['length'] > 0.3 and mark_price > mark_price_prev):
                make_trade = True
