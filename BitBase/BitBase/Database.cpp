...
std::unique_ptr<TickTableRead> Database::open_tick_table_read(const std::string& exchange, const std::string& symbol)
{
...
TickTableRead::~TickTableRead(void)
{
    file.close();
}
...
std::unique_ptr<Intervals> Database::get_intervals(const std::string& exchange, const std::string& symbol, const time_point_ms& timestamp_start, const time_point_ms& timestamp_end, const std::chrono::milliseconds interval)
{
    ...
    file.seekg(interval_offset * sizeof(Interval));

    auto timestamp = timestamp_start;
    auto database_intervals = std::make_unique<Intervals>(BitBase::Bitmex::first_timestamp, interval);
