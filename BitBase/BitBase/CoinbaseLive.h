...
{
public:
    CoinbaseLive(sptrDatabase database, tick_data_updated_callback_t tick_data_updated_callback);

    CoinbaseLiveState get_state(void);
    void start(void);
    void shutdown(void);
...
private:
    ...
    std::atomic<CoinbaseLiveState> state;
...
using uptrCoinbaseLive = std::unique_ptr<CoinbaseLive>;
