...
enum class CoinbaseProTickState {
    idle,
    downloading
};
...
{
public:
    CoinbaseProTick(sptrDatabase database, tick_data_updated_callback_t tick_data_updated_callback);

    CoinbaseProTickState get_state(void);
    void start(void);
    void shutdown(void);
...
using uptrCoinbaseProTick = std::unique_ptr<CoinbaseProTick>;
