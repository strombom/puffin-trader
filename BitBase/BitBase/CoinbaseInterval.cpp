...
CoinbaseInterval::CoinbaseInterval(sptrDatabase database) :
    database(database), interval_data_thread_running(true)
{
    interval_data_worker_thread = std::make_unique<std::thread>(&CoinbaseInterval::interval_data_worker, this);
}

void CoinbaseInterval::shutdown(void)
{
...
void CoinbaseInterval::update(void)
{
    interval_data_condition.notify_one();
}

void CoinbaseInterval::interval_data_worker(void)
{
    while (interval_data_thread_running) {
        ...
        const auto symbols = database->get_attribute(BitBase::Coinbase::exchange_name, "symbols", std::unordered_set<std::string>{});
        for (auto&& symbol : symbols) {
            if (std::find(BitBase::Coinbase::symbols.begin(), BitBase::Coinbase::symbols.end(), symbol) == BitBase::Coinbase::symbols.end()) {
                // Symbol is not enabled, continue with next symbol
                continue;
            }
            for (auto&& interval : BitBase::Coinbase::Interval::intervals) {
...
void CoinbaseInterval::make_interval(const std::string& symbol, std::chrono::milliseconds interval)
{
    const auto interval_name = std::to_string(interval.count());
    auto timestamp = database->get_attribute(BitBase::Coinbase::exchange_name, symbol + "_interval_" + interval_name + "_next_timestamp", BitBase::Coinbase::first_timestamp);
    const auto last_timestamp = timestamp + interval * (BitBase::Coinbase::Interval::batch_size - 1);
    auto next_tick_idx = database->get_attribute(BitBase::Coinbase::exchange_name, symbol + "_interval_" + interval_name + "_next_tick_idx", 0ll);
    auto tick_table = database->open_tick_table_read(BitBase::Coinbase::exchange_name, symbol);
    ...
    while (timestamp <= last_timestamp && timer.elapsed() < BitBase::Coinbase::Interval::batch_timeout) {
        ...
        for (auto&& buy : buys) {
            ...
            while (step_idx < BitBase::Coinbase::Interval::steps.size() && accum_vol_buy > BitBase::Coinbase::Interval::steps[step_idx]) {
        ...
        for (auto&& sell : sells) {
            ...
            while (step_idx < BitBase::Coinbase::Interval::steps.size() && accum_vol_sell > BitBase::Coinbase::Interval::steps[step_idx]) {
    ...
    database->extend_interval_data(BitBase::Coinbase::exchange_name, symbol, interval, intervals_data, timestamp, next_tick_idx);
}
