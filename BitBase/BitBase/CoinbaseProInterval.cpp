...
CoinbaseProInterval::CoinbaseProInterval(sptrDatabase database) :
    database(database), interval_data_thread_running(true)
{
    interval_data_worker_thread = std::make_unique<std::thread>(&CoinbaseProInterval::interval_data_worker, this);
}

void CoinbaseProInterval::shutdown(void)
{
...
void CoinbaseProInterval::update(void)
{
    interval_data_condition.notify_one();
}

void CoinbaseProInterval::interval_data_worker(void)
{
    while (interval_data_thread_running) {
        ...
        const auto symbols = database->get_attribute(BitBase::CoinbasePro::exchange_name, "symbols", std::unordered_set<std::string>{});
        for (auto&& symbol : symbols) {
            if (std::find(BitBase::CoinbasePro::symbols.begin(), BitBase::CoinbasePro::symbols.end(), symbol) == BitBase::CoinbasePro::symbols.end()) {
                // Symbol is not enabled, continue with next symbol
                continue;
            }
            for (auto&& interval : BitBase::CoinbasePro::Interval::intervals) {
...
void CoinbaseProInterval::make_interval(const std::string& symbol, std::chrono::milliseconds interval)
{
    const auto interval_name = std::to_string(interval.count());
    auto timestamp = database->get_attribute(BitBase::CoinbasePro::exchange_name, symbol + "_interval_" + interval_name + "_next_timestamp", BitBase::CoinbasePro::first_timestamp);
    const auto last_timestamp = timestamp + interval * (BitBase::CoinbasePro::Interval::batch_size - 1);
    auto next_tick_idx = database->get_attribute(BitBase::CoinbasePro::exchange_name, symbol + "_interval_" + interval_name + "_next_tick_idx", 0ll);
    auto tick_table = database->open_tick_table_read(BitBase::CoinbasePro::exchange_name, symbol);
    ...
    while (timestamp <= last_timestamp && timer.elapsed() < BitBase::CoinbasePro::Interval::batch_timeout) {
        ...
        for (auto&& buy : buys) {
            ...
            while (step_idx < BitBase::CoinbasePro::Interval::steps.size() && accum_vol_buy > BitBase::CoinbasePro::Interval::steps[step_idx]) {
        ...
        for (auto&& sell : sells) {
            ...
            while (step_idx < BitBase::CoinbasePro::Interval::steps.size() && accum_vol_sell > BitBase::CoinbasePro::Interval::steps[step_idx]) {
    ...
    database->extend_interval_data(BitBase::CoinbasePro::exchange_name, symbol, interval, intervals_data, timestamp, next_tick_idx);
}
