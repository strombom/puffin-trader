...
e_series = {
    ...
    24: [1.0, 1.1, 1.2, 1.3, 1.5, 1.6, 1.8, 2.0, 2.2, 2.4, 2.7, 3.0, 3.3, 3.6, 3.9, 4.3, 4.7, 5.1, 5.6, 6.2, 6.8,
         7.5, 8.2, 9.1],
    48: [1.00, 1.05, 1.10, 1.15, 1.21, 1.27, 1.33, 1.40, 1.47, 1.54, 1.62, 1.69, 1.78, 1.87, 1.96, 2.05, 2.15, 2.26,
         2.37, 2.49, 2.61, 2.74, 2.87, 3.01, 3.16, 3.32, 3.48, 3.65, 3.83, 4.02, 4.22, 4.42, 4.64, 4.87, 5.11, 5.36,
         5.62, 5.90, 6.19, 6.49, 6.81, 7.15, 7.50, 7.87, 8.25, 8.66, 9.09, 9.53],
    96: [1.00, 1.02, 1.05, 1.07, 1.10, 1.13, 1.15, 1.18, 1.21, 1.24, 1.27, 1.30, 1.33, 1.37, 1.40, 1.43, 1.47, 1.50,
    ...
    192: [1.00, 1.01, 1.02, 1.04, 1.05, 1.06, 1.07, 1.09, 1.10, 1.11, 1.13, 1.14, 1.15, 1.17, 1.18, 1.20, 1.21, 1.23,
...
def make_e_series_range(start, end, e_num):
    decade = 100
    while start < 10 ** decade:
        decade -= 1

    idx = 0
    while e_series[e_num][idx] * 10 ** decade < start:
        idx += 1

    series = []
    while e_series[e_num][idx] * 10 ** decade < end:
        series.append(e_series[e_num][idx] * 10 ** decade)
        idx += 1
        if idx == len(e_series[e_num]):
            decade += 1
            idx = 0
...
if __name__ == '__main__':
    print(make_e_series_range(0.003, 0.010, 192))
