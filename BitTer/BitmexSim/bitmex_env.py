...
class BitmexEnv:
    ...
    def step(self, action):
        ...
        #if action == BitmexActions.market_buy_1:
        #    order_leverage = 1.0
        # if action == BitmexActions.market_buy_2:
        #     order_leverage = 2.0
        # elif action == BitmexActions.market_buy_3:
        #     order_leverage = 3.0
        # elif action == BitmexActions.market_sell_1:
        #     order_leverage = -1.0
        # elif action == BitmexActions.market_sell_2:
        #     order_leverage = -2.0

        mark_price = self.runner_clock.ie_prices[self.step_idx]
        leverage = self.simulator.get_leverage(mark_price=mark_price)

        if order_leverage != 0 and abs(order_leverage - leverage) > 0.2:
        ...
        if action == BitmexActions.limit_buy_near:
            ...
            action_price = self.runner_clock.ie_prices[self.step_idx] + 0.5
        elif action == BitmexActions.limit_sell_delta:
            action_price = self.runner_clock.ie_prices[self.step_idx + 1] - 0.5
        ...
        if action == BitmexActions.limit_buy_near or action == BitmexActions.limit_buy_delta:
            if self.runner_clock.ie_prices_min[self.step_idx + 1] < action_price:
                order_leverage = self.max_leverage
        elif action == BitmexActions.limit_sell_near or action == BitmexActions.limit_sell_delta:
        ...
        self.log['leverage'].append(leverage)
    ...
    def reset(self):
        ...
        order_leverage = random.choice([5, -6])
        order_size = self.simulator.calculate_order_size(leverage=order_leverage, mark_price=self.start_price)
        self.simulator.market_order(order_contracts=order_size, mark_price=self.start_price)
